...
describe.only('Reading Lists API', () => {
  it('can add a blog to reading list', async () => {
    ...
    assert.strictEqual(response.data.blogId, createdBlogId)  //changed to camelCase
    assert.strictEqual(response.data.userId, testData.users[0].id) //changed to camelCase
